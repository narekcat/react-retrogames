...
const postGame = (req, res) => {
    // We assing the game info to a empty game and send a message back if no errors
...
// Delete the game by the given ID
const deleteGame = (req, res) => {
    // We remove the game by the given ID and send message if no errors
    Game.remove(
        { _id: req.params.id },
        err => {
            if (err) {
